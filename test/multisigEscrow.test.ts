...
import {Owner, StatefulMultiSig} from "../src/contracts/statefullMultisig";
import {getDefaultSigner} from "../scripts/helper";
import {
  bsv,
  FixedArray,
  MethodCallOptions,
...
import {myAddress} from "../scripts/myPrivateKey";
...
describe("Test SmartContract `StatefulMultiSig`", () => {
  ...
  it("should pass paying out if threshold reached.", async () => {
    ...
    const callContract = async () =>
      statefulMultiSig.methods.pay(
        // Method call options:
        {
          changeAddress: await statefulMultiSig.signer.getDefaultAddress(),
        } as MethodCallOptions<StatefulMultiSig>
      );
    return expect(callContract()).not.rejected;
  });
  ...
  it("should fail adding invalid sig.", async () => {
    ...
    return expect(callContract()).to.be.rejectedWith(/signature check failed/);
  });
